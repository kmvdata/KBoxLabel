...
- **数据库存储**：完全采用 SQLite3 数据库存储项目配置和标注数据，替代原有的 .kolo 文件
- **多项目支持**：支持同时打开和管理多个项目窗口
...
- **多项目支持**：可以同时打开多个项目窗口进行工作
- **最近项目**：应用会记住最近打开的项目，方便快速访问
- **项目数据存储**：所有项目数据（包括标注信息、类别配置等）都存储在 SQLite 数据库中
...
2. 选择导出目录
3. 系统将自动转换所有标注数据为 COCO 格式

#### YOLO 格式

1. 在菜单栏选择 "导出" -> "导出为 YOLO"
2. 系统将自动转换所有标注数据为 YOLO 格式
...
KBoxLabel 现在完全使用 SQLite 数据库存储所有项目数据，包括：

- 标注信息（kolo_item 表）
- 标注类别（annotation_category 表）
- 项目配置（kv_config 表）

数据库文件存储在项目目录下的 `.kboxlabel` 文件夹中。
